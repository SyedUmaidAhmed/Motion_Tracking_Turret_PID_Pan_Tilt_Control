...
def rotate5 (direction, strength):  
...
def grab_frame_display(run_flag, send_frame_queue, receive_contour_queue, p_start_turn, p_end_turn, p_start_lock, p_end_lock):
    ...
    while (run_flag.value):
        ...
        frame_hsvtwo = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        
        
        exceed_threshold = cv2.absdiff(frame_hsv, frame_hsvtwo)
        
        frame_hsv = frame_hsvtwo
        returnBoolean, threshold_image = cv2.threshold(exceed_threshold,THRESHOLD_SENSITIVITY, 255,cv2.THRESH_BINARY)
        ...
        if (calibrationNow):
            ...
            if ((delta_time_ms > 30) and exceed_threshold and (send_frame_queue.qsize() < 4)):
                ...
                send_frame_queue.put(mask) # Put mask in queue No idea yet
...
def process_frame_1(run_flag, send_frame_queue, receive_contour_queue, p_start_turn, p_end_turn, p_start_lock, p_end_lock):
    while (run_flag.value):
        ...
        if ((not send_frame_queue.empty()) and (p_start_turn.value == 1)):
            ...
            # 2. Extract contour
            contours,h=cv2.findContours(threshold_image,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
